...
function ditherImage(pixels, imageWidth, imageHeight){
	...
	//dither image
	const performanceResults = App.Timer.megapixelsPerSecond('Ordered dithering performance', imageWidth * imageHeight, ()=>{
...
onmessage = (e)=>{
    ...
    if(imageHeader === null){
        const data = new Uint16Array(messageData);
        imageHeader = {
            width: data[0],
            height: data[1],
        };
    ...
    ditherImage(pixels, imageHeader.width, imageHeader.height);
    imageHeader = null;
};